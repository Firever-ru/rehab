...
export default function AdminDashboard() {
  ...
  const [content, setContent] = useState({ title: '', description: '', quotes: [], hero_image: null });
  ...
  return (
    <main className="dashboard">
      <aside>
        <div className="admin-logo">ВД</div>
        <b>ВТОРОЕ ДЫХАНИЕ</b>
        <nav>
      ...
      <section className="dash-main">
        ...
        <section id="content" className="panel">
          ...
          {content.quotes.map((q, i) => (
            <div className="quote-row" key={i}>
              ...
              <button type="button" className="outline" onClick={() => removeQuote(i)}>
                Удалить
              </button>
            </div>
          ))}
          <button type="button" className="outline" onClick={addQuote}>
            + Добавить цитату
          </button>
          ...
          {content.hero_image && (
            <img src={content.hero_image} alt="Текущее главное фото" className="hero-preview" />
          )}
        ...
        <section id="requests" className="panel">
          <div className="panel-title">
            <h2>Заявки</h2>
            <button className="outline" onClick={downloadExcel}>
              Скачать Excel
            </button>
          </div>
          <div className="filters">
            ...
            <button className="outline" onClick={() => loadApplications(dateFrom, dateTo)}>
              Показать
            </button>
            <button className="outline" onClick={deleteRange}>
              Удалить за период
            </button>
          </div>
          <table>
